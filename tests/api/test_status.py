...
from kokudaka.api import API_VERSION
from kokudaka.api.status import StatusView
from tests import KokudakaTestCase


class StatusAPITest(KokudakaTestCase):
    ...
    @patch('kokudaka.api.status.os.environ')
    ...
    @patch('kokudaka.api.status.subprocess.run')
    @patch('kokudaka.api.status.os.environ')
    ...
    @patch('kokudaka.api.status.platform.uname')
    ...
    @patch('kokudaka.api.status.sys.version')
    ...
    @patch('kokudaka.api.status.sys.modules')
    ...
    @patch('kokudaka.api.status.logger.info')
    ...
    @patch('kokudaka.api.status.StatusView.modules', new_callable=PropertyMock)
    def test_startup_without_modules(self, mock_mods):
        """Test the startup method without a module list."""
        mock_mods.return_value = {}
        expected = 'INFO:kokudaka.api.status:Modules: None'

        with self.assertLogs('kokudaka.api.status', level='INFO') as logger:
            StatusView().startup()
